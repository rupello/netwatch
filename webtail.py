...
def check_file():
    ...
    if not line:
        ...
        print "File refresh"
        (n,ts,src,d) = syslogparser.process_message(line)
        if d.startswith('ACCEPT'):
            fields = syslogparser.parse_fields(d)
            message = 'ACCEPT %s %s:%d --> %s:%d, IN=%s OUT=%s' % (
                                            fields.get('PROTO',''), 
                                            fields.get('SRC',''), 
                                            int(fields.get('SPT',0)), 
                                            fields.get('DST',''), 
                                            int(fields.get('DPT',0)), 
                                            fields.get('IN',''), 
                                            fields.get('OUT','') 
                                            )
            for element in LISTENERS:
                element.write_message(message)
